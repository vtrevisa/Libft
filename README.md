...
| [**ft\_strlcat**](https://github.com/vtrevisa/Libft/blob/master/Src/ft_strlcat.c) | ```size_t	ft_strlcat(char *dest, const char *src, size_t size);``` |  Appends **size** - strlen(**dst**) - 1 bytes of the NUL-terminated string **src** to the end of **dest**, NUL-terminating the result. |
